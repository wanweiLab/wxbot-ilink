...
项目已经具备可运行的管理后台。下面的截图展示管理员登录页和多 Bot 管理页，访问者可以先看到当前产品的实际形态，再了解已经完成的运行时能力和后续路线。
...
当前版本已经提供一套可用于构建微信 Bot 和企业 AI 接入服务的 Java 运行时和管理后台能力。
...
- Spring Boot 自动配置、Reactor 异步门面和 TestKit
- 多 Bot 绑定、运行状态查看、在线状态、实例状态和链路健康展示
- Bot 的启动、停止、解绑和测试消息操作

SDK 对外提供 at-least-once 消息语义，不承诺 exactly-once。业务处理存在副作用时，应使用 `messageId` 或业务唯一键实现幂等。

## Roadmap，即将开发

下面的能力会围绕企业员工、企业 Agent 和微信 Bot 之间的关系逐步建设。

### 1. 员工管理

建立企业员工的统一管理入口，支持员工信息、组织归属、绑定状态和使用状态管理。后续会补充员工与微信身份的绑定、角色和权限配置、可用 Agent 范围、离职停用和操作审计。

### 2. 企业定制 Agent 管理

为企业提供 Agent 的注册、创建和配置能力。管理员可以为不同部门和业务场景定义 Agent，配置提示词、企业知识范围、业务工具、服务对象和权限边界，并逐步加入版本管理、发布、回滚和责任人设置。

### 3. Bot 连接 Agent

在现有 Bot 管理能力上增加 Agent 连接配置。管理员可以把 Bot 绑定到指定 Agent，设置员工、群组或业务场景的路由规则，让微信消息进入正确的企业 Agent，并保留会话上下文、权限判断、消息状态和运行审计。
